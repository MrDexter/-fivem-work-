		...
		if modname[mod][2] == "Turbo" then
			ESX.TriggerServerCallback('tuner:itemamount', function(amount)
				if not (IsToggleModOn(vehicle, 18)) == modid[stage][3] and amount >= modid[stage][1] then
					TaskStartScenarioInPlace(ped, "PROP_HUMAN_BUM_BIN", 0, true)
					Passed = Progress(modname[mod][2], modid[stage][2], 5000)
					if Passed then
			...
			ESX.TriggerServerCallback('tuner:itemamount', function(amount)
				if not (GetVehicleMod(vehicle, modname[mod][1]) == modid[stage][1]) and amount > 0 then
					...
					print("HIII")
					showText = true
					modifacation = "FLIPPER"
					stage = 1
					while showText == true do
						Citizen.Wait(100)
					end
					
					Citizen.Wait(1000)
					print(stage)
					Passed = Progress(modname[mod][2], "Stage " .. modid[stage][1] + 1, 5000)
					if Passed then
...
Citizen.CreateThread(function() 
	while true do
		...
		if showText == true then
		 	print(stage, modifacation)
			drawTxt('~h~Select ' .. modifacation .. ' Stage:~h~~n~' .. stageName[stage], 4, 1, 0.5, 0.8, 0.75, 255,255,255,255 )
		...
		if showText == true and IsControlJustReleased(0, 201) and stage < 4 then
			showText = false
		elseif showText == true and IsControlJustReleased(0, 175) and stage < 4 then
			stage = stage + 1
		elseif showText == true and IsControlJustReleased(0, 174) and stage > 0 then
			stage = stage - 1
