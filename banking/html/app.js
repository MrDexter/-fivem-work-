...
function AddTransaction(trans_id, account, amount, time, note, why, trans_name, plName, account_name)
{
...
Listeners["refresh_accounts"] = function() {
    for (var key in ValidAccounts) {
       ...
       let data = ValidAccounts[key];
        ...
        if (data && data === "business" || data === "organisation")
        {
...
Listeners["refresh_balances"] = function(data) {
    ...
    for (var i = 0; i < tbl.length; i++)
    {
        ...
        let account_name = (tTbl.type === "business" && tTbl.name || tTbl.type === "organisation" && tTbl.name || "Personal Account");
        let temp_name = account_name.replace(/"|'/g,'');
        let temp_Name = account_name.replace(/\s+/g, '');
